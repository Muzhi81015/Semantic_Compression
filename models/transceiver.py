...
class MultiHeadedAttention(nn.Module):
    def __init__(self, num_heads, d_model, dropout=0.1):
        ...
        super(MultiHeadedAttention, self).__init__()
        assert d_model % num_heads == 0
        self.tcn = TemporalConvNet(d_model, [d_model] * 4)  # 定义一个TCN层
        self.fc = nn.Linear(d_model, d_model)  # 定义一个全连接层，其中输入的大小为hiddensize，输出的大小为outputsize
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x = x.permute(0, 2, 1)  # TCN expects [batch, features, sequence_length]
        x = self.tcn(x)
        x = x.permute(0, 2, 1)  # Convert back to [batch, sequence_length, features]
